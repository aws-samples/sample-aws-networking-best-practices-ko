...
*   :material-file-document: **AWS Direct Connect 설명서**

    ---

    연결, 가상 인터페이스, Direct Connect Gateway, SiteLink, MACsec을 다루는 전체 서비스 설명서입니다.

    [:octicons-arrow-right-24: 설명서](https://docs.aws.amazon.com/directconnect/latest/UserGuide/Welcome.html)
...
Site-to-Site VPN은 하이브리드 연결을 구축하는 가장 빠른 방법입니다. 전용 회선이 필요하지 않은 사이트의 기본 경로로, 데이터 마이그레이션과 같이 단기간 연결이 필요한 경우에, 그리고 MACsec을 사용할 수 없고 레이어 3 암호화가 필요할 때 Direct Connect 위에 IPsec 오버레이로 일반적으로 활용됩니다.
...
*   :material-speedometer: **표준 및 대용량 터널 대역폭**

    ---

    표준 터널은 터널당 최대 1.25 Gbps를 제공합니다. 대용량 터널은 터널당 최대 5 Gbps를 제공하여 고처리량 워크로드에서 복잡한 ECMP 본딩이 필요 없습니다. 대용량 터널은 Transit Gateway VPN 및 AWS Cloud WAN VPN 어태치먼트에서 사용할 수 있습니다.

*   :material-office-building-marker: **다수의 원격 사이트를 위한 VPN Concentrator**

    ---

    다수의 저대역폭(100 Mbps 미만) VPN 연결을 단일 어태치먼트로 집계하는 Transit Gateway 어태치먼트로, Concentrator당 최대 100개 사이트와 5 Gbps 집계 대역폭을 지원합니다. AWS에서 여러 가상 Concentrator 어플라이언스를 관리해야 했던 수십 개의 지사(소매, 숙박, 의료)를 보유한 분산형 기업을 위해 설계되었습니다.
...
*   :material-router: **Direct Connect를 통한 프라이빗 IP VPN**
    ...
    공용 인터넷 대신 Direct Connect 연결을 통해 도달 가능한 프라이빗 IP 주소에서 VPN을 종료합니다. 터널 엔드포인트를 인터넷에 노출하지 않고 Direct Connect 회선 위에 IPsec 암호화를 원할 때 유용합니다.

*   :material-network: **BGP를 이용한 동적 라우팅**

    ---

    각 터널의 BGP를 통해 온프레미스 라우터가 AWS 프리픽스를 동적으로 학습하고 온프레미스 프리픽스를 AWS에 광고할 수 있습니다. BGP를 실행하지 않는 디바이스에는 정적 라우팅이 지원되지만, 단순하지 않은 토폴로지에는 BGP를 강력히 권장합니다.
...
모든 VPN 연결은 두 개의 IPsec 터널을 제공하지만, 두 터널 모두 동일한 연결 내의 AWS 엔드포인트에서 종료됩니다. 프로덕션 복원력을 위해 **두 개 이상의 VPN 연결**을 프로비저닝하고, 서로 다른 온프레미스 라우터에서 종료하며, 각 라우터가 **서로 다른 ISP**를 통해 AWS에 도달하도록 구성하여 단일 공급자 장애로 두 경로가 동시에 중단되지 않도록 합니다. 모든 터널에서 BGP를 사용하면 AWS가 경로를 자동으로 학습하고 장애 조치를 수행하며, 세션 상태를 통해 도달 가능성을 보고하고, BGP 속성으로 트래픽을 조정할 수 있습니다. 정적 라우팅은 이러한 기능을 모두 사용할 수 없으므로 프로덕션 환경에는 거의 적합하지 않습니다. 온프레미스 디바이스가 BGP를 지원하지 않는 경우, 우회 방법을 구축하기보다 디바이스를 교체하는 것이 올바른 해결책입니다.
...
가상 프라이빗 게이트웨이에 연결된 VPN은 정확히 하나의 VPC에만 도달합니다. Transit Gateway 또는 AWS Cloud WAN 코어 네트워크에 연결된 VPN은 허브가 라우팅하는 모든 VPC에 도달하며, 라우팅 테이블 또는 네트워크 정책을 통해 세분화가 적용됩니다. VPC가 두 개 이상인 환경에서는 Transit Gateway 또는 AWS Cloud WAN에서 VPN을 종료하세요.

이를 통해 대용량 터널(터널당 5 Gbps), Accelerated VPN, 다중 VPN 어태치먼트에 걸친 ECMP도 활성화할 수 있으며, 이러한 기능은 가상 프라이빗 게이트웨이 VPN에서는 사용할 수 없습니다.

#### 다수의 저대역폭 사이트에는 VPN Concentrator 활용 {#use-vpn-concentrator-for-many-low-bandwidth-sites}

[VPN Concentrator](https://docs.aws.amazon.com/vpc/latest/tgw/tgw-vpn-concentrator-attachments.html)는 다수의 지사 사용 사례를 해결합니다. 사이트 수는 많지만 각각의 대역폭 요구사항은 낮은 경우, 사이트별로 전용 VPN 연결을 프로비저닝하면 Transit Gateway 어태치먼트가 과도하게 늘어나고 워크로드에 맞지 않는 연결당 비용이 발생합니다. VPN Concentrator 어태치먼트는 최대 100개 사이트(각각 100 Mbps 미만)를 단일 Transit Gateway 어태치먼트로 집계하며, Concentrator당 5 Gbps 집계 대역폭과 Transit Gateway당 최대 5개의 Concentrator를 지원합니다.

저대역폭 프로파일에 해당하는 원격 사이트가 약 25개 이상인 경우(소매 체인, 호텔, 의료 지사, 분산된 현장 사무소) VPN Concentrator를 활용하세요. Concentrator는 BGP 전용 라우팅을 실행하고 AZ 이중화 엔드포인트를 제공하므로, 운영 측면에서는 사이트별 연결을 개별 관리하는 대신 일관된 라우팅 정책과 보안 규칙이 모든 연결 사이트에 적용되는 하나의 어태치먼트처럼 보입니다. 사이트 수가 적거나 100 Mbps 이상의 대역폭이 필요한 사이트에는 표준 Site-to-Site VPN 연결을 사용하세요.

#### 원거리 사이트에는 Accelerated VPN 활용 {#use-accelerated-vpn-for-distant-sites}

AWS 리전에서 멀리 떨어진 지사나 데이터 센터의 경우, Accelerated VPN은 공용 인터넷 대신 AWS Global Accelerator 엣지 네트워크를 통해 트래픽을 라우팅합니다. 지터 및 처리량 개선 효과는 지연 시간에 민감하거나 대역폭에 민감한 하이브리드 워크로드에서 의미 있게 나타나며, 연결별 토글로 활성화할 수 있습니다.

Accelerated VPN은 AWS 리전이 사이트와 다른 대륙에 있거나 사이트와 리전 간의 인터넷 라우팅이 역사적으로 불안정한 경우에 가장 유용합니다.
...
CloudWatch는 터널별로 `TunnelState`, `TunnelIpAddress`, BGP 세션 상태 지표를 게시합니다. 터널 상태 전환 및 BGP 세션 플랩 횟수에 대한 알람을 설정하세요. 단일 터널 중단은 주기적으로 발생할 수 있으며(AWS는 터널 엔드포인트에 대한 계획된 유지 관리를 수행합니다), 동일한 연결의 두 터널이 몇 분 이상 모두 중단되면 장애 상황입니다.

모든 연결에서 [Site-to-Site VPN 로그](https://docs.aws.amazon.com/vpn/latest/s2svpn/monitoring-logs.html)를 활성화하세요. 이 로그는 IKE 협상, IPsec 터널 설정, 데드 피어 감지, BGP 라우팅 활동을 캡처하며, 협상 실패나 세션 불안정 문제를 해결하는 데 필요한 정보를 제공합니다. 로그 없이는 터널 설정 실패를 진단할 때 일반적으로 온프레미스 디바이스의 로그만 확인해야 하지만, 로그를 활성화하면 대화의 양쪽 내용을 한 곳에서 확인할 수 있습니다.
...
* MACsec을 사용할 수 없거나(10 Gbps 미만의 호스팅 연결) 컴플라이언스상 레이어 3 암호화가 필요하여 Direct Connect 위에 암호화된 오버레이가 필요한 경우
* 인터넷 기반 암호화 처리량으로 충분한 일회성 데이터 마이그레이션과 같이 연결이 단기간인 경우
* 실질적으로 필요한 Direct Connect 회선 수보다 많은 소규모 지사를 다수 연결하는 경우(대규모에서는 일반적으로 VPN Concentrator가 적합한 도구)

Site-to-Site VPN은 공용 인터넷이 보장할 수 없는 수준의 예측 가능한 지연 시간이 필요한 경우, 인터넷 이그레스 비용이 과도해질 정도의 대용량 데이터를 전송하는 경우, 또는 컴플라이언스상 암호화된 오버레이가 아닌 물리적 전용 회선이 명시적으로 요구되는 경우에는 **적합하지 않습니다**.
...
*   :material-file-document: **AWS Site-to-Site VPN 설명서**

    ---

    터널 구성, BGP, Transit Gateway 및 AWS Cloud WAN 어태치먼트, 대용량 터널, Accelerated VPN을 다루는 전체 서비스 설명서입니다.

    [:octicons-arrow-right-24: 설명서](https://docs.aws.amazon.com/vpn/latest/s2svpn/VPC_VPN.html)

*   :material-file-document-outline: **Accelerated VPN**

    ---

    Accelerated VPN을 활성화하는 방법과 원거리 사이트의 처리량을 향상시키는 경우에 대한 설명입니다.

    [:octicons-arrow-right-24: 설명서](https://docs.aws.amazon.com/vpn/latest/s2svpn/accelerated-vpn.html)
...
*   :material-currency-usd: **AWS Site-to-Site VPN 요금**
    ...
    연결 시간당 요금과 데이터 전송 비용으로 구성되며, 표준 터널과 대용량 터널 및 Accelerated VPN에 대한 별도 요금이 적용됩니다.
...
*   :material-tunnel: **GRE 및 터널리스 Connect**
    ...
    GRE는 SD-WAN 어플라이언스와 AWS 측 간의 터널에서 BGP 및 데이터 트래픽을 캡슐화합니다. 터널리스 Connect(AWS Cloud WAN 전용)는 캡슐화를 완전히 생략하므로 BGP가 어플라이언스와 Cloud WAN 간에 직접 실행됩니다. 터널리스는 GRE 오버헤드를 제거하여 일반적으로 피어당 더 높은 처리량을 제공합니다.
...
SD-WAN 벤더가 AWS 내에서 가상 어플라이언스를 실행하도록 권장하는 경우(일반적인 패턴), Transit Gateway 또는 AWS Cloud WAN에서 VPC 기반 Connect 어태치먼트를 사용하세요. Transit VPC에 어플라이언스를 배포하고, VPC를 선택한 서비스에 연결한 다음, 그 위에 Connect 어태치먼트를 구성합니다. SD-WAN 오버레이는 인터넷 또는 Direct Connect를 통해 온프레미스 디바이스를 어플라이언스에 연결합니다.

SD-WAN 어플라이언스가 온프레미스에 유지되면서 AWS와 직접 피어링하는 경우, Direct Connect 언더레이(Direct Connect Gateway 경유)를 사용하는 Transit Gateway Connect 어태치먼트를 사용하세요. 온프레미스 SD-WAN 디바이스는 Direct Connect 회선을 통해 Transit Gateway에 GRE 피어를 설정합니다. 이 방식은 AWS 내의 가상 어플라이언스 계층을 완전히 제거하지만, SD-WAN이 종료되는 모든 위치에 Direct Connect가 필요합니다. 이 패턴은 현재 Transit Gateway 전용이며, AWS Cloud WAN Connect는 Direct Connect Gateway 언더레이를 지원하지 않습니다.

#### 세그먼트별 Connect 피어 이중화 설계 {#design-for-connect-peer-redundancy-per-segment}

Connect 어태치먼트는 최대 4개의 Connect 피어를 지원하며, 각 피어는 2개의 BGP 세션을 가집니다. 이중화를 위해 어태치먼트당 최소 2개의 Connect 피어를 프로비저닝하고, 서로 다른 SD-WAN 어플라이언스 또는 서로 다른 온프레미스 디바이스에 종료되도록 구성하세요. AWS는 모든 Connect 피어에서 두 BGP 세션을 모두 구성할 것을 강력히 권장합니다. 단일 세션만 실행하면 AWS 유지 관리 이벤트 중 BGP 컨트롤 플레인 장애에 대한 보호가 제거됩니다.

SD-WAN 트래픽을 여러 AWS 환경(예: 동일한 Connect 어태치먼트를 통한 프로덕션 및 비프로덕션)으로 세분화하는 경우, 각 세그먼트는 일반적으로 경로가 적절히 필터링된 자체 Connect 피어가 필요합니다.
...
워크로드가 여러 퍼블릭 클라우드에 걸쳐 운영되는 사례가 늘고 있습니다. 한 클라우드에는 데이터베이스, 다른 클라우드에는 분석 플랫폼, 그리고 AWS에서 실행되는 애플리케이션이 두 클라우드를 모두 사용하는 형태가 대표적입니다. 과거에는 AWS VPC를 다른 클라우드 네트워크에 연결하려면 경로를 직접 구성해야 했습니다. 코로케이션 시설까지 Direct Connect를 프로비저닝하고, 상대 클라우드의 동등한 서비스와 크로스 커넥트를 연결한 뒤, 양쪽의 물리적 구성과 BGP 설정을 직접 유지 관리해야 했습니다. [AWS Interconnect](https://docs.aws.amazon.com/interconnect/latest/userguide/what-is-interconnect.html)는 AWS와 피어 클라우드가 엔드 투 엔드로 프로비저닝하고 유지 관리하는 관리형 직접 클라우드 간 연결을 제공함으로써 이러한 방식을 바꿉니다.

지원되는 리전 쌍이 존재하는 경우 AWS Interconnect를 기본 선택지로 사용하십시오. 지원되는 리전 쌍이 없는 경우에는 파트너 기반 크로스 커넥트나 인터넷 VPN을 검토하기 전에, 각 클라우드 내부에서 지원되는 쌍으로부터 연결을 확장할 수 있는지 먼저 확인하십시오. 커버리지 공백은 대부분 라우팅 문제이지, 경로를 직접 구축해야 하는 이유가 아닙니다.

AWS Interconnect는 AWS VPC와 다른 클라우드 공급자 네트워크 간에 프라이빗 고속 연결을 생성하는 완전 관리형 서비스입니다. AWS Interconnect 멀티 클라우드는 리전 쌍 기반으로 동작합니다. 각 Interconnect는 하나의 AWS 리전과 피어 클라우드의 하나의 리전을 연결하며, 연결이 필요한 리전 쌍마다 Interconnect를 프로비저닝합니다. 지원되는 클라우드 공급자와 리전 쌍은 계속 확장되고 있으므로, 특정 쌍을 기준으로 설계하기 전에 [리전 가용성](https://docs.aws.amazon.com/interconnect/latest/userguide/region-availability.html)에서 최신 목록을 확인하십시오. AWS 측에서 Interconnect는 Direct Connect Gateway에 연결되며, Direct Connect Gateway는 Transit Gateway, AWS Cloud WAN 코어 네트워크, 또는 가상 프라이빗 게이트웨이와 연결됩니다.

코로케이션에서 직접 구성하는 방식과 달리, AWS Interconnect는 물리적 크로스 커넥트, BGP 설정, VLAN 프로비저닝을 추상화합니다. AWS 리전, 피어 클라우드의 리전, 필요한 대역폭, 공급자를 선택하면 AWS와 공급자가 양쪽을 자동으로 프로비저닝합니다. 용량은 재프로비저닝 없이 확장하거나 축소할 수 있습니다.
...
*   :material-cloud-sync: **AWS ↔ 다른 클라우드 직접 연결**
    ...
    단일 관리 객체(*interconnect*)가 Direct Connect Gateway를 피어 클라우드의 연결 지점에 연결합니다. 트래픽은 AWS 글로벌 백본을 통해 공급자로 전달되며 중간 인터넷 홉 없이 직접 핸드오프됩니다.

*   :material-shield-lock-outline: **완전 관리형 MACsec 암호화**

    ---

    모든 물리적 링크는 AWS 라우터와 공급자 라우터 사이에서 IEEE 802.1AE MACsec으로 암호화되며, AWS와 공급자가 키 수명 주기를 관리합니다. 사용자가 CKN/CAK 쌍을 생성, 교체, 연결할 필요가 없으며, 이 기능이 없는 대역폭 계층도 존재하지 않습니다. 트래픽은 암호화 세션이 활성화된 경우에만 흐르므로 암호화되지 않은 경로는 존재하지 않습니다.

*   :material-check-all: **기본 제공 최대 복원력**

    ---

    모든 Interconnect는 독립적인 전원과 네트워킹을 갖춘 물리적으로 분리된 최소 두 개의 시설에 걸쳐 이중화 디바이스로 프로비저닝됩니다. 멀티 클라우드 연결은 ECMP 로드 밸런싱을 사용하는 4개 연결 모델을 채택하므로, 계획된 유지 관리나 디바이스 장애 시에도 최소 하나의 링크가 유지됩니다. 단일 Interconnect는 **월 99.99% 가동 시간 [SLA](https://aws.amazon.com/interconnect/sla/)**를 제공합니다.
...
*   :material-router-network: **AWS 연결 지점으로서의 Direct Connect Gateway**
    ...
    Interconnect의 AWS 측은 Direct Connect Gateway에 연결되며, Direct Connect Gateway는 Transit Gateway, AWS Cloud WAN 코어 네트워크, 또는 가상 프라이빗 게이트웨이와 연결됩니다.

*   :material-clock-fast: **빠른 프로비저닝**

    ---

    Interconnect는 일반적으로 AWS와 공급자 간의 생성-수락 워크플로를 통해 몇 분 내에 프로비저닝됩니다. 크로스 커넥트 티켓, BGP 설정, VLAN 협상이 필요하지 않습니다.

</div>

### AWS Interconnect 모범 사례 {#aws-interconnect-best-practices}

#### 지원되는 리전 쌍과 공급자에는 AWS Interconnect를 기본으로 사용 {#use-aws-interconnect-as-the-default-for-supported-region-pairs-and-providers}

AWS Interconnect가 필요한 AWS 리전과 피어 클라우드 리전을 지원하는 경우, AWS Interconnect를 기본 선택지로 사용해야 합니다. 파트너 기반 Direct Connect 대안과 비교했을 때 운영상의 차이는 상당합니다. 크로스 커넥트가 없고, 라우터에서 BGP를 구성할 필요가 없으며, 용량 변경 시 공급자와 조율할 필요가 없고, 모든 링크에 MACsec이 적용되어 키를 직접 관리하지 않아도 됩니다.

다른 방법을 검토해야 하는 이유는 커버리지 문제이므로, 특정 쌍이 목록에 있다고 가정하지 말고 [리전 가용성](https://docs.aws.amazon.com/interconnect/latest/userguide/region-availability.html)을 확인하십시오. 목록에 없는 경우, AWS Interconnect를 포기하기 전에 지원되는 쌍이 각 클라우드 내부에서 해당 리전에 도달할 수 있는지 확인하십시오([필요한 리전에 지원되는 쌍이 없는 경우](#no-supported-pair-for-the-regions-you-need) 참조). 파트너 기반 방식과 인터넷 기반 VPN은 전혀 지원되지 않는 클라우드이거나, 백본 경로는 작동하지만 기존 코로케이션 또는 캐리어 인프라로 인해 두 백본 간 리전 간 데이터 전송보다 크로스 커넥트 하나를 추가하는 것이 더 효율적인 경우에 적합합니다.

#### 단일 Interconnect를 복원력 단위로 취급 {#treat-a-single-interconnect-as-the-resilient-unit}

99.99% [SLA](https://aws.amazon.com/interconnect/sla/)는 하나의 Interconnect에 적용됩니다. 이중화가 객체 내부에 포함되어 있기 때문입니다. 최소 두 개의 시설에 걸쳐 AWS와 공급자가 사전 프로비저닝한 4개 연결 ECMP 패브릭이 이를 구현합니다. Direct Connect는 직접 관리해야 하는 다중 사이트 이중화 배포를 통해서만 동일한 수준의 약정에 도달할 수 있으며, 단일 Direct Connect 연결은 95% 계층에 해당합니다. 흔한 실수는 이러한 복원력 툴킷 습관을 그대로 가져와 이중화를 위해 동일한 리전 쌍에 두 번째 Interconnect를 프로비저닝하는 것입니다. 두 Interconnect 모두 동일한 패브릭을 사용하므로 비용만 두 배가 될 뿐 약정 수준은 높아지지 않습니다. Interconnect는 다른 리전 쌍이 필요할 때만 추가하십시오.

경계 지점 이후의 모든 것은 사용자의 책임입니다. Direct Connect Gateway 연결, Transit Gateway 또는 Core Network Edge, 그리고 워크로드가 이에 해당합니다. SLA 적용 자격을 얻으려면 엔드포인트가 두 개 이상의 가용 영역에 걸쳐 있어야 하므로, 단일 AZ 대상은 가용성 위험이자 약정 범위 밖에 해당합니다.

***핵심 인사이트:*** *AWS Interconnect는 복원력을 직접 설계해야 하는 것에서 자동으로 상속받는 것으로 전환합니다. 설계 질문이 "회선과 위치를 몇 개로 할 것인가"에서 "Direct Connect Gateway 뒤의 모든 것이 앞단의 Interconnect만큼 가용한가"로 바뀝니다.*

#### AWS Cloud WAN을 사용하여 AWS 리전과 Interconnect 리전을 분리 {#use-aws-cloud-wan-to-decouple-aws-region-from-interconnect-region}

가상 프라이빗 게이트웨이나 Transit Gateway는 Direct Connect Gateway를 통해서도 자신의 AWS 리전에 있는 Interconnect에만 도달할 수 있습니다. 해당 Direct Connect Gateway를 **AWS Cloud WAN**에 연결하면 이 제약이 사라집니다. 모든 Core Network Edge가 모든 Interconnect에 도달할 수 있으므로, 하나의 Interconnect가 코어 네트워크의 모든 리전에 있는 워크로드를 지원할 수 있습니다.

리전 제약은 권고 사항이 아닌 강제 사항입니다. 원격 리전의 Transit Gateway에 이미 연결된 Direct Connect Gateway에 Interconnect를 연결하려고 하면 즉시 실패합니다.

피어 클라우드 측은 다른 문제입니다. Interconnect가 쌍을 이루는 피어 클라우드 리전은 Interconnect 자체에 의해 고정되며, AWS Cloud WAN으로는 이를 변경할 수 없습니다. 다른 피어 클라우드 리전으로 트래픽을 전달하려면 해당 리전과 쌍을 이루는 Interconnect를 사용하거나, 쌍을 이루는 리전에서 해당 클라우드가 제공하는 리전 간 연결을 통해 홉을 추가해야 합니다. 이는 AWS Cloud WAN이 처리할 수 없는 부분으로, 해당 클라우드 측에서 직접 확인해야 합니다. 이 홉이 적합한 답인 경우에 대해서는 [필요한 리전에 지원되는 쌍이 없는 경우](#no-supported-pair-for-the-regions-you-need)를 참조하십시오.

#### Interconnect의 도달 범위가 확장될수록 요금 계층에 주의 {#watch-the-pricing-tier-as-the-interconnects-reach-grows}

AWS Interconnect에는 GB당 데이터 전송 요금이 없습니다. 두 가지 입력값에 따라 시간당 요금이 결정됩니다. 프로비저닝한 대역폭과, Interconnect의 로컬 리전과 트래픽을 사용하는 리전 간의 지리적 거리를 반영하는 1(로컬)에서 5(최대 범위)까지의 [요금 계층](https://docs.aws.amazon.com/interconnect/latest/userguide/interconnect-pricing.html)입니다. Interconnect는 사용 중인 모든 경로를 포함하는 가장 낮은 계층을 구독하므로, 가장 먼 리전이 모든 경로의 요금을 결정합니다. 로컬로 연결된 Transit Gateway나 가상 프라이빗 게이트웨이는 항상 계층 1입니다. 연결은 Interconnect의 로컬 리전에서만 가능하기 때문입니다. AWS Cloud WAN의 경우, 라우팅 이점에는 비용 영향이 따릅니다. Core Network Edge를 추가하면 계층이 재평가되므로, 새로운 AWS 리전이 추가되면 Interconnect가 여러 계층 위로 이동할 수 있습니다.

개념 증명(PoC) 작업을 위해 AWS Interconnect 멀티 클라우드는 일반 공급 클라우드 공급자별로 AWS 리전당 무료 로컬(계층 1) 500 Mbps Interconnect 하나를 제공합니다. 이 무료 계층은 AWS 측만 해당하므로, 다른 공급자의 요금도 별도로 확인하십시오.

#### 처음부터 IPv6를 계획 {#plan-ipv6-from-the-start}

AWS Interconnect는 Direct Connect Gateway에 대한 IPv4 및 IPv6 BGP 세션을 모두 지원합니다. 처음부터 Interconnect에서 IPv4와 함께 IPv6를 활성화하여, 피어 클라우드의 IPv6 지원이 워크로드 측에서 준비되었을 때 전송 계층이 이미 갖춰져 있도록 하십시오.

### AWS Interconnect 사용 시기 {#when-to-use-aws-interconnect}

AWS Interconnect는 다음과 같은 경우에 적합한 선택입니다.

* 필요한 AWS 리전과 피어 클라우드 리전이 지원되는 쌍이거나, 각 클라우드 내에서 지원되는 쌍으로부터 도달 가능한 경우.
* IPsec 터널을 직접 운영하고 유지 관리하지 않고 암호화된 클라우드 간 트래픽을 원하는 경우.
* 코로케이션에서의 크로스 커넥트와 두 클라우드 및 시설 간의 BGP, VLAN 조율을 피하고 싶은 경우.
* 정적 용량에 고정되지 않고 워크로드 변화에 따라 대역폭이 변경될 것으로 예상되는 경우.

AWS Interconnect가 **적합하지 않은** 경우:

* 필요한 클라우드가 전혀 지원되지 않는 경우 — 대신 [파트너 기반 Direct Connect](#partner-based-aws-direct-connect-to-another-cloud) 또는 [클라우드 간 Site-to-Site VPN](#aws-site-to-site-vpn-between-clouds)을 사용하십시오.
* 이미 두 클라우드 모두에 도달하는 코로케이션 인프라나 캐리어 패브릭을 운영 중이며, 크로스 커넥트 하나를 추가하는 것이 양쪽의 리전 간 데이터 전송보다 비용이 적게 드는 경우.
* 쌍을 이루는 리전을 통한 추가 홉이 지연 시간 예산을 초과하는 경우.

#### 필요한 리전에 지원되는 쌍이 없는 경우 {#no-supported-pair-for-the-regions-you-need}

지원되지 않는 리전 쌍은 지원되지 않는 클라우드와 다릅니다. 지원되는 쌍에 Interconnect를 연결하고, 각 클라우드 내부에서 나머지 구간을 처리하십시오.

AWS 측에서는 이미 해결책이 있습니다. AWS Cloud WAN 코어 네트워크에 연결된 Direct Connect Gateway를 사용하면 모든 Core Network Edge가 해당 게이트웨이의 모든 Interconnect에 도달할 수 있으므로, Interconnect의 리전이 워크로드가 실행되는 리전일 필요가 없습니다. Transit Gateway를 사용하는 경우, 리전 간 Transit Gateway 피어링을 통해 워크로드 리전에서 Interconnect가 로컬인 리전으로 이동할 수 있습니다. 두 경로 모두 비용이 발생합니다. AWS Cloud WAN 경로는 Interconnect의 요금 계층을 높일 수 있고, Transit Gateway 피어링 경로는 리전 간 데이터 전송 요금이 추가됩니다.

피어 클라우드 측에서는 해당 공급자의 문서에서 자체 리전 간 트래픽 전달 방식을 확인하십시오. 메커니즘, 제약 사항, 요금은 클라우드마다 다르므로, 이 설계를 확정하기 전에 쌍을 이루는 리전에서 대상 리전에 도달 가능한지, 그리고 비용이 얼마인지 확인하십시오.

| 고려 사항 | 각 클라우드 내부에서 확장 | 서드파티 상호 연결 공급자 |
| --- | --- | --- |
| 관리 주체 | AWS와 피어 클라우드, Interconnect 객체 하나 | 사용자, 외부 연결 공급자, 두 클라우드 |
| 비용 구조 | 가장 먼 리전이 결정하는 계층의 시간당 Interconnect 요금(Interconnect 자체에는 GB당 요금 없음), 선택적으로 각 클라우드의 리전 간 데이터 전송 요금 | 공급자 포트 및 패브릭 요금과 각 클라우드의 프라이빗 연결 요금 |
| 지연 시간 | 예측 가능하지만 쌍을 이루는 리전을 통한 홉이 추가됨 | 연결 공급자가 두 대상 리전 근처에 거점을 보유한 경우 더 짧을 수 있음 |
| 암호화 | Interconnect 홉에 MACsec 적용, 각 리전 간 구간은 해당 클라우드 자체 네트워크 내에서 유지 | 직접 구축 필요, 모든 포트에 MACsec 또는 IPsec 오버레이 |
| 프로비저닝 | 클라우드 콘솔과 API를 통해 몇 분 내 완료 | 크로스 커넥트와 BGP를 포함하여 수일에서 수주 소요 |

***핵심 인사이트:*** *누락된 리전 쌍은 조달 문제이기 전에 라우팅 문제입니다. 서드파티, 크로스 커넥트, 직접 BGP 구성을 도입하기 전에 각 클라우드가 마지막 구간을 내부적으로 처리할 수 있는지 먼저 확인하십시오.*

### 기타 멀티 클라우드 옵션 {#other-multi-cloud-options}

이러한 패턴은 AWS Interconnect가 도달하지 못하는 경우, 즉 전혀 지원되지 않는 클라우드이거나 백본 경로는 기술적으로 가능하지만 경제성이나 지연 시간 측면에서 직접 경로를 구축하는 것이 유리한 경우에 적용됩니다.

#### 파트너 기반 AWS Direct Connect를 통한 다른 클라우드 연결 {#partner-based-aws-direct-connect-to-another-cloud}

오랫동안 사용되어 온 패턴입니다. 피어 클라우드의 동등한 서비스(Google Cloud Partner Interconnect, Microsoft Azure ExpressRoute, Oracle FastConnect)도 함께 위치한 Direct Connect 로케이션에서 AWS Direct Connect 연결을 프로비저닝하고, 크로스 커넥트를 주문하거나 가상 상호 연결 파트너의 패브릭을 사용하여 두 클라우드를 연결한 뒤, 양쪽에서 BGP를 설정하여 클라우드 간 프리픽스를 광고합니다.

이 방식은 AWS Interconnect가 아직 지원하지 않는 리전과 클라우드 쌍에서 프라이빗 고대역폭 연결을 제공하며, 이미 Direct Connect 회선과 코로케이션 인프라를 운영 중이고 동일한 시설을 확장하여 두 번째 클라우드를 연결하려는 경우에 적합한 선택일 수 있습니다.

AWS Interconnect와 비교한 트레이드오프:

* **관리해야 할 물리적·논리적 크로스 커넥트**. 두 클라우드와 시설, 총 세 당사자가 관여합니다.
* **양쪽에서 BGP 설정 필요**. 경로가 엔드 투 엔드로 관리되지 않으므로 BGP를 직접 설정하고, 세션을 유지하며, 결합된 동작을 디버깅해야 합니다.
* **암호화는 사용자 책임**. 코로케이션 시설의 크로스 커넥트는 기본적으로 MACsec으로 암호화되지 않습니다. 컴플라이언스상 암호화가 필요한 경우 크로스 커넥트 위에 IPsec을 구성해야 하며, 이는 AWS Interconnect가 피하는 터널 오버헤드를 다시 도입하게 됩니다.
* **용량 변경에 더 많은 시간 소요**. 대역폭 확장은 일반적으로 콘솔에서 조정하는 것이 아니라 크로스 커넥트를 재프로비저닝해야 합니다.

이 패턴은 AWS Interconnect가 해당 클라우드나 리전 쌍을 지원하지 않고, 트래픽 볼륨이나 지연 시간 요구 사항이 인터넷 기반 대안을 허용하지 않는 경우에 적합한 선택입니다.

#### 클라우드 간 AWS Site-to-Site VPN {#aws-site-to-site-vpn-between-clouds}

가장 간단한 멀티 클라우드 옵션이며, 때로는 소량 또는 단기 워크로드에 유일한 선택지이기도 합니다. AWS Transit Gateway 또는 AWS Cloud WAN 코어 네트워크에서 피어 클라우드의 VPN 게이트웨이로 Site-to-Site VPN을 구성합니다. BGP가 터널 위에서 실행되어 클라우드 간 프리픽스를 전달합니다.

인터넷 기반 VPN은 설정이 빠르고(물리적 프로비저닝 불필요), 소량 워크로드에 저렴하며, 광범위하게 지원됩니다. 다만 인터넷 기반이므로 성능이 공용 인터넷 상태에 따라 달라지며, 양쪽에서 IPsec 처리량을 소비합니다.

AWS Interconnect를 사용할 수 없고 워크로드가 인터넷 기반 성능을 허용하는 경우, 또는 파트너 기반 Direct Connect 설정을 정당화하기 어려운 소량 클라우드 간 트래픽에 사용하십시오.

#### 멀티 클라우드 옵션 비교 {#comparing-the-multi-cloud-options}

| 항목 | AWS Interconnect | 파트너 기반 Direct Connect | 클라우드 간 Site-to-Site VPN |
| --- | --- | --- | --- |
| 전송 방식 | AWS 글로벌 백본, 피어 클라우드로 직접 핸드오프 | 코로케이션 시설의 전용 크로스 커넥트 | 공용 인터넷 |
...
| 프로비저닝 시간 | 몇 분 | 수일에서 수주 | 몇 분 |
| 용량 변경 | 탄력적, 온디맨드 | 크로스 커넥트 재프로비저닝 필요 | 각 클라우드에서 탄력적 |
| 비용 구조 | 대역폭과 거리 계층에 따른 Interconnect당 시간당 요금, GB당 요금 없음 | 포트 시간, 크로스 커넥트 요금, 데이터 전송 아웃 요금 | 연결당 시간당 요금과 데이터 전송 요금 |
| SLA | Interconnect당 [99.99%](https://aws.amazon.com/interconnect/sla/) | 배포 모델에 따라 다름: 다중 사이트 이중화 99.99%, 다중 사이트 비이중화 99.9%, 단일 연결 95% | VPN 연결당 [99.95%](https://aws.amazon.com/vpn/site-to-site-vpn-sla/) |
| 현재 커버리지 | 지원되는 클라우드와의 선택된 AWS 리전 쌍, 확장 중 | 코로케이션 인프라 범위 내 모든 주요 클라우드 | 모든 주요 클라우드 |
| 최적 사용 사례 | 지원되는 클라우드 쌍과 리전, 프로덕션 멀티 클라우드 워크로드 | AWS Interconnect가 아직 지원하지 않는 클라우드 쌍과 리전, 기존 코로케이션 배포 | AWS Interconnect가 아직 지원하지 않는 클라우드 쌍이나 리전의 소량 또는 단기 워크로드 |
...
*   :material-file-document: **AWS Interconnect 문서**
    ...
    멀티 클라우드 및 라스트 마일 서비스, 개념, 연결 지점, 지원되는 리전 쌍을 포함한 전체 서비스 문서입니다.
...
*   :material-currency-usd: **AWS Interconnect 요금**
    ...
    프로비저닝된 대역폭과 거리 기반 요금 계층에 따른 시간당 요금이며, GB당 데이터 전송 요금은 없습니다. 크로스 커넥트 요금은 파트너 기반 대안과 달리 관리형 서비스에 포함됩니다.

    [:octicons-arrow-right-24: 요금](https://aws.amazon.com/interconnect/multicloud/pricing/)

</div>

## 하이브리드 및 멀티 클라우드 스택 구축 {#building-your-hybrid-and-multi-cloud-stack}
